...
void* ledFunction(void *arg)
{
	...
	while (!data.endFlag)			// endFlag가 0일때 ledControl
	{
...
void* hc04Function(void *arg)
{
	...
	pinMode(echo, INPUT);
	digitalWrite(trig, LOW);
	while (!data.endFlag)			// endFlag가 0일때 hc04Control
	{
		ret = hc04Control();
		data.hc04_dist = ret;
...
void* userThread(void *arg)
{
	...
	pthread_create(&t_id[0], NULL, ledFunction, 0);			// LED 쓰레드 생성
	pthread_create(&t_id[1], NULL, hc04Function, 0);		// HC04 쓰레드 생성
	...
	while ((str_len = read(clnt_sock, &buf, sizeof(buf))) != 0)
	{
		...
		case WR_LED:
			...
			break;
		case RD_HC04:
			data.cmd = WR_DIST;		//	cmd값을 WR_DIST로 바꿔서, ledClient의 read_routine을 통해 거리가 출력되도록 한다.
			write(clnt_sock, &data, sizeof(data));
			printf("data.dist:%f\n", data.hc04_dist);
			break;
		case RD_IMG:
			data.cmd = WR_IMG;		// cmd값을 WR_IMG로 바꿔서, ledClient의 read_routine을 통해 파일이 저장되도록 한다.
			write(clnt_sock, &data, sizeof(data));
	...
	data.endFlag = 1;				// endFlag를 1로 설정하여 ledControl과 hc04Control이 실행되지 않게 한다.

	pthread_detach(t_id[0]);		// t_id[0] 쓰레드 종료시 자원 반납
	pthread_detach(t_id[1]);		// t_id[1] 쓰레드 종료시 자원 반납
...
int main(int argc, char **argv)
{
	...
	pthread_t thread_LED;							// LED를 위한 쓰레드
	pthread_t t_id;
	int str_len;
	struct sockaddr_in serv_adr, clnt_adr;
	int clnt_adr_sz;								// 클라이언트 주소 사이즈

	data.endFlag = 0;								// endFlag를 0으로 초기화
	...
	// STEP 1. socket생성
	serv_sock = socket(PF_INET, SOCK_STREAM, 0);	// 소켓 생성, PF_INET: IPv4, 프로토콜 체계를 표현할 때 사용, SOCK_STREAM: TCP사용
	memset(&serv_adr, 0, sizeof(serv_adr));
	serv_adr.sin_family = AF_INET;					// AF_INET: IPv4, 주소체계를 표현할 때 사용
	serv_adr.sin_addr.s_addr = htonl(INADDR_ANY);	// htonl: long intger 데이터를 네트워크 byte order로 변경, 주소정보를 초기화한다.
	serv_adr.sin_port = htons(atoi(argv[1]));		// htons: short intger 데이터를 네트워크 byte order로 변경, argv[1]로 포트번호를 받는다.
	...
	while (1)
	{
		...
		clnt_sock = accept(serv_sock, (struct sockaddr*)&clnt_adr, &clnt_adr_sz);		// 클라이언트 소켓에 연결을 받아들인다.
